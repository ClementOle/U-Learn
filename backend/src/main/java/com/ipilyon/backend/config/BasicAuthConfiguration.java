...
public class BasicAuthConfiguration extends WebSecurityConfigurerAdapter {
	...
	private static class AuthentificationLoginSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
		...
		public void onAuthenticationSuccess(HttpServletRequest request,
			...
			s.setAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY, sctx);

			response.setStatus(HttpServletResponse.SC_OK);
			response.setHeader("user", ((UsernamePasswordAuthenticationToken) authentication).getPrincipal().toString());
		}
	}
